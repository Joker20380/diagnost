...
- разделить canonical DTC и наблюдения из загруженных отчётов;
- прекратить обновление общего справочника данными отдельного автомобиля;
- добавить parser version, content hash, provenance и replay;
- заменить двусмысленный `ai_generated_at` на method/version metadata;
- добавить fixture PDF и regression tests для Launch parser;
- сделать повторный импорт идемпотентным;
- обеспечить неизменяемость подписанного осмотра на уровне domain/service layer;
- превратить QA warnings из логов в записи `QAEvent`;
- документировать существующие local changes и привести ветку к контролируемому состоянию.

Критерий выхода:

- импорт одного файла воспроизводим и не загрязняет knowledge base;
- подписанный результат нельзя молча изменить;
- parser regressions обнаруживаются тестами;
- для каждого результата известны источник и версия обработки.
